...
def seg_and_patch(source, save_dir, patch_save_dir, mask_save_dir, stitch_save_dir, 
	...
	for cancer_folder in ["Lymph_node_metasis_absent", "Lymph_node_metasis_present"]:
		for slide in os.listdir(source + '/' + cancer_folder):
			dict_map_sub_folder[slide] = cancer_folder

	slides = [slide for slide in slides if slide[0] != '.']
	...
	for i in range(total):
		...
		# Inialize WSI
		full_path = os.path.join(source, dict_map_sub_folder[slide], slide)
		try:
		...
		if stitch:
			...
			if os.path.isfile(file_path):
				...
				heatmap.resize((width // 8, height // 8)).save(stitch_path)
...
if __name__ == '__main__':
	...
	seg_params = {'seg_level': -1, 'sthresh': 4, 'mthresh': 7, 'close': 1, 'use_otsu': args.use_ostu,
				  'keep_ids': 'none', 'exclude_ids': 'none'}
	filter_params = {'a_t':1, 'a_h': 1, 'max_n_holes':16}
