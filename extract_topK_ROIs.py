...
if __name__ == '__main__':
    ...
    os.makedirs(os.path.join(args.data_h5_dir, 'topk_patches_512'),exist_ok=True)
    ...
    ckpt_path = args.exp_code
    ckpt_path = os.path.join('./results', ckpt_path + '_s2021', 's_0_checkpoint_best_f1.pt')
    ...
    curr_feat_ids = [os.path.basename(fname).split('.')[0] for fname in glob.glob(os.path.join(args.feat_dir, 'pt_files', '*'))]
    curr_slide_ids = [os.path.basename(fname).split('.')[0] for fname in glob.glob(os.path.join(args.data_h5_dir, 'patches', '*'))]
    curr_slide_ids = [item for item in curr_slide_ids if item in curr_feat_ids]
    ...
    for bag_candidate_idx in range(total):
        ...
        print(slide_id)
        output_path = os.path.join(args.data_h5_dir, "topk_patches_512", slide_id + '.pt')
        ...
        dataset = Whole_Slide_Bag_FP_SAVE(file_path=patch_file_path, wsi=wsi, select_idx = topk_indexes, target_patch_size=args.target_patch_size)
